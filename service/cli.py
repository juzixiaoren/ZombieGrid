...
def handle_delete_strategy():
    """处理删除策略的逻辑"""
    db_manager = DBSessionManager()# 创建新的 DBSessionManager 实例
    configs = []
    try:
        with db_manager as session:
            configs = session.query(GridConfig).order_by(GridConfig.id).all()
    ...
    # 再次确认删除
    prompt = (f" 警告：确定要删除策略 '{strategy_name}' (ID: {config_id_to_delete}) 吗？\n"
...
def handle_import_market_data():
    ...
    try:
        convert_success = excel_to_json(excel_file_path, json_file_path) # 调用转换函数
...
def handle_delete_market_data():
    ...
    prompt = (f" 警告：确定要删除导入批次 ID {selected_import_id} "
...
def handle_backtest():
    ...
    print("-" * 40 + "\n")
    try:
        backtest = BackTest(grid_data, grid_strategy, initial_capital)
        result = backtest.run_backtest() # 内部打印流水
    ...
    #保存结果到 Excel
        if result: # 确保回测成功执行了
            ...
            #策略配置数据
            config_dict = selected_config.to_dict()
